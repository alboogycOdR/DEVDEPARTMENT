...
3. Ensure worktrees exist: `powershell -File scripts/worktree.ps1 -Action create` (or `git worktree add ../wt-<repo-name>-grok main` / `../wt-<repo-name>-codex main` directly — worktrees are namespaced per project so multiple DEVDEPARTMENT projects never collide on the same parent-directory path).
4. Launch builders:
