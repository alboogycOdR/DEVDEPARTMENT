...
- **Exclude `board/*.json` from that copy.** Those are *generated* board data, not template infrastructure — copying them imports the pack repo's own stale `board/DEVDEPARTMENT.json` / `board/projects.json` verbatim into the new project, which then renders another project's dead state as if it were this one's. Copy the `board/` scaffolding only, then regenerate with `python3 scripts/board_publisher.py` once PLAN.md exists. (Found during the tower onboarding, 2026-08-26.)
- **Node/ESM projects — `package.json` with `"type": "module"`, the default for Vite/React/most modern JS stacks:** `hooks/*.js` are CommonJS, so `node hooks/run-tests.js` fails with `require is not defined in ES module scope`. Do **not** convert the hooks. Use Node's own scoping mechanism instead: create `hooks/package.json` containing exactly `{"type": "commonjs"}`, which re-scopes that directory without touching the project's module type. Verify with `node hooks/run-tests.js` before moving on. (Found during the tower onboarding, 2026-08-26.)
...
- `PLAN.md`, `REVIEW.md` — only if absent. If you bootstrap a fresh `PLAN.md` instead of copying the pack's, it MUST satisfy the **current** `scripts/validate_plan.py` — never reconstruct one from an older template or from git history. Fields the validator now requires were optional in earlier versions (notably `Updated_By`/`Updated_At`, which must be non-empty **even on the TASK-001 example block** — em-dash placeholders there fail validation). Run `python3 scripts/validate_plan.py` and get a clean exit before Step 2. (Found during the tower onboarding, 2026-08-26.)
