...
- v4.6 — Pack sync system: `sync-manifest.json` (declares every path framework_owned / project_owned / merge_special — ownership is data, not per-sync judgment) + `scripts/sync_from_pack.py`, a three-way sync (pack vs project vs `.devteam/sync_state.json` baseline) that brings already-onboarded projects up to date with pack improvements — the gap onboard.md's deliberate add-only design leaves open (observed live: a project still running the original v1.0.0 protocol against a v4.5 pack). Updates clean files, adds missing ones, flags locally-customized files as conflicts (never silently clobbered; `--adopt-pack` to resolve pack-ward), merges CLAUDE.md's marker section and autopilot.json's keys add-only (project tuning always preserved), never touches project-owned state (PLAN.md/REVIEW.md/INSTINCTS.md/specs/dossiers), and never edits `.claude/settings.json`/`.codex/config.toml` (manual-only, reported). Dry-run by default, byte-exact copies, onboarding now writes the sync baseline. See docs/SYNC.md. Python tests: 507, Node hook tests: 28.
- v4.7 — Configurable builder registry: `autopilot.json`'s `builders` key becomes the single source of truth for the roster (dual-shape: legacy flat array forever accepted; registry object with `active`/`defined`, per-unit cli/model/auth/suffixes/briefing/usage_provider). New `scripts/builder_registry.py` loader (fail-open for "which builders exist", fail-closed for resolving a specific unit); dispatch.sh/.ps1 rewritten registry-driven (argv = unit ID with a permanent legacy cli-name shim; per-CLI invocation quirks stay as a 3-row family table; `auth.mode=config_dir` sets CLAUDE_CONFIG_DIR scoped to the launch only); validator/supervisor/budget registry-driven; **security fix**: the firewall now fails CLOSED on a set-but-unknown DEVTEAM_UNIT (exit 2 via the verdict path — previously it silently granted unrestricted ORCH permissions). Ships **S5B** (second Claude Pro-plan login) defined-but-inactive; activation procedure + live CLAUDE_CONFIG_DIR verification in docs/BUILDER_REGISTRY.md. Briefing templating evaluated and deferred (S5B reuses S5's briefing); per-account usage probing is the named follow-up (increment 9). Python tests: 534, Node hook tests: 32.
- v4.8 — Builder identity via native agents: new `.claude/agents/devteam-builder.md` defines the builder role properly, and registry field `identity: "agent"` dispatches claude-CLI units with `--agent` instead of prepending the old IDENTITY OVERRIDE preamble. That preamble opened with "Ignore CLAUDE.md's ORCH role assignment entirely" — the exact shape of a prompt-injection attempt, which a safety-trained model is *correct* to treat with suspicion (observed live: a builder refused a dispatch as injection when quoting truncated it, `deacac3`). The agent file instead frames CLAUDE.md's ORCH section as "your counterpart, not you", so there is nothing to override. Opt-in: `preamble` remains the default, byte-identical, until `--agent` is live-verified (procedure in docs/BUILDER_REGISTRY.md); an unrecognised value degrades to `preamble` rather than stranding a builder. Also: legacy flat-array configs now normalize to the same entry shape as registry configs (found by its own new test — consumers previously saw different fields depending on config shape); S5B activation gained hard prerequisites (Claude Code ≥ 2.1.225 for the headless-OAuth fix; watch for the new workspace-trust prompt on a fresh worktree); and new docs/BACKLOG.md records six evaluated-and-deferred capabilities, each with the trigger that would change the answer. Python tests: 579, Node hook tests: 36.
