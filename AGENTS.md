...
5. **Work on your task branch in your worktree** (`task/TASK-NNN-gb`, `-cx`, or `-s5`). Never commit **code** to `main` — merging is ORCH's alone, after review. **PLAN.md is the deliberate exception:** your PLAN.md-only coordination commits (claim, status transitions, Progress_Notes, `needs_review`) must land on `main` immediately via `git add PLAN.md && git commit && git push . HEAD:main`, because a claim nobody else can see is not a claim. Full procedure in your briefing. Conventional Commits, every message ends with `[TASK-NNN]`.
